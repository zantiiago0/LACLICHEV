#ifndef org_apache_lucene_search_spans_SpanOrQuery$SpanOrWeight_H
#define org_apache_lucene_search_spans_SpanOrQuery$SpanOrWeight_H

#include "org/apache/lucene/search/spans/SpanWeight.h"

namespace java {
  namespace io {
    class IOException;
  }
  namespace lang {
    class Class;
  }
  namespace util {
    class List;
    class Set;
    class Map;
  }
}
namespace org {
  namespace apache {
    namespace lucene {
      namespace index {
        class Term;
        class TermContext;
        class LeafReaderContext;
      }
      namespace search {
        namespace spans {
          class Spans;
          class SpanOrQuery;
          class SpanWeight$Postings;
        }
        class IndexSearcher;
      }
    }
  }
}
template<class T> class JArray;

namespace org {
  namespace apache {
    namespace lucene {
      namespace search {
        namespace spans {

          class SpanOrQuery$SpanOrWeight : public ::org::apache::lucene::search::spans::SpanWeight {
          public:
            enum {
              mid_init$_951807d35a6a3145,
              mid_extractTermContexts_c63a2704a45ba212,
              mid_extractTerms_34e67f8c9ede20a4,
              mid_getSpans_7cf21bcb381674b4,
              max_mid
            };

            static ::java::lang::Class *class$;
            static jmethodID *mids$;
            static bool live$;
            static jclass initializeClass(bool);

            explicit SpanOrQuery$SpanOrWeight(jobject obj) : ::org::apache::lucene::search::spans::SpanWeight(obj) {
              if (obj != NULL)
                env->getClass(initializeClass);
            }
            SpanOrQuery$SpanOrWeight(const SpanOrQuery$SpanOrWeight& obj) : ::org::apache::lucene::search::spans::SpanWeight(obj) {}

            SpanOrQuery$SpanOrWeight(const ::org::apache::lucene::search::spans::SpanOrQuery &, const ::org::apache::lucene::search::IndexSearcher &, const ::java::util::Map &, const ::java::util::List &);

            void extractTermContexts(const ::java::util::Map &) const;
            void extractTerms(const ::java::util::Set &) const;
            ::org::apache::lucene::search::spans::Spans getSpans(const ::org::apache::lucene::index::LeafReaderContext &, const ::org::apache::lucene::search::spans::SpanWeight$Postings &) const;
          };
        }
      }
    }
  }
}

#include <Python.h>

namespace org {
  namespace apache {
    namespace lucene {
      namespace search {
        namespace spans {
          extern PyTypeObject PY_TYPE(SpanOrQuery$SpanOrWeight);

          class t_SpanOrQuery$SpanOrWeight {
          public:
            PyObject_HEAD
            SpanOrQuery$SpanOrWeight object;
            static PyObject *wrap_Object(const SpanOrQuery$SpanOrWeight&);
            static PyObject *wrap_jobject(const jobject&);
            static void install(PyObject *module);
            static void initialize(PyObject *module);
          };
        }
      }
    }
  }
}

#endif
